...
res<- predict(logitmod,down_train,type = "response")
...
No<- numeric()
for(i in cut_off) {
  t<-table(ActualValue=down_train$Class,PredictValue=res>i)
...
print(result)
write.xlsx(result,file = "emp.xlsx", sheet = "2", append = TRUE)
