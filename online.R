...
cluster_centers
library(xlsx)
write.xlsx(cluster_centers, "cluster.xlsx",sheetName = "5",append = TRUE)
...
#hclustering
preprocessed<-customers[,c("recency.log","frequency.log","monetary.log")]
op<-hclust(dist(preprocessed),method="average")
plot(op)
h<-6
clusterCut<-cutree(op,h)
customers[,'Hclust_cat']<-clusterCut
customers[,'Hclust_cat']<-factor(customers[,'Hclust_cat'],levels = c(1:h))
library(plyr)
cluster_centers <- ddply(customers, .(customers$Hclust_cat), summarize,
                         monetary=round(median(monetary),2),frequency=round(median(frequency),2),
                         recency=round(median(recency), 0),cl_count=length(recency.log))
# to rename the cluster column
names(cluster_centers)[names(cluster_centers)=="customers$Hclust_cat"] <- "Cluster"
cluster_centers
library(car)
library(rgl)
scatter3d(x = customers$frequency.log,
          ...
          grid = FALSE,
          axis.col = c("black", "black", "black"))
write.xlsx(cluster_centers, "cluster.xlsx",sheetName = "h6",append = TRUE)
